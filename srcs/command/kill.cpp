...
/*   Updated: 2023/05/23 22:26:46 by miguelangel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
    ...
    {
        this->send_message(this->fds[i].fd,ERR_NOPRIVILEGES(this->get_host())); // ERR_NOPRIVILEGES
        return;
    }
    ...
    if (aux.size() == 2)
    {
        for (int x = 0;x < run->n_active_fds; x++)
            ...
            {
                //this->send_message(this->fds[i].fd,ERR_NEEDMOREPARAMS(this->get_host())); // Necesito enviar el mensaje de eliminado
                return;
            }
            else if (this->clients[x].getnick() == aux[0] && this->clients[x].getop() == true)
            {
                this->send_message(this->fds[i].fd,ERR_NOPRIVS(this->get_host())); // ERR_NOPRIVS
                return;
            }
