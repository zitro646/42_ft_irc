...
/*   Updated: 2023/05/24 19:24:43 by mortiz-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void server::analize_msg (int i, std::string str , data_running *run)
{
	...
	std::string cmd[12] = {"NICK","USER","JOIN","PING","PART","LIST","QUIT","PRIVMSG","OPER","KILL","TOPIC","NOTICE"};
	server::funptr function[12] = {&server::NICK, &server::USERNAME, &server::JOIN, &server::PONG,&server::PART,&server::LIST, &server::QUIT,&server::MSG,&server::OPER,&server::KILL,&server::TOPIC,&server::NOTICE};// &server::extract_JOIN,
